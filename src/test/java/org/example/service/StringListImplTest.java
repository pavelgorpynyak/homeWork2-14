...
class StringListImplTest {
    ...
    @ParameterizedTest
    @NullSource
    public void shouldThrowIfNull( String input ) {
    ...
    @ParameterizedTest
    @CsvSource({"0,2", "1,2"})
    public void doesntThrowExceptionWhenAddByIndexInBounds(int index, String item) {
        assertDoesNotThrow(() -> out.add(index, item));
    }

    @ParameterizedTest
    @CsvSource({"0,2", "1,2"})
    public void doesntThrowWhenAddByIndexInBounds(int index, String toAdd) {
    ...
    public void shouldRemoveByObjectCorrectly() {
        ...
        assertEquals("2", out.get(0));
        assertEquals("3", out.get(1));
        assertDoesNotThrow(() -> out.remove("3"));
        assertEquals("2", out.get(0));
        assertThrows(IllegalArgumentException.class, () -> out.get(1));
    }
    ...
    @ParameterizedTest
    @ValueSource(strings = {"1","2", "3", "4", "5", "-1", "null"})
    public void shouldReturnCorrectGet(String input) {
        out.add(input);
        assertEquals("1", out.get(0));
    ...
    @ParameterizedTest
    @ValueSource(strings = {"2", "3", "4", "5", "-1", "null"})
    public void shouldThrowIfElementDoesntExistRemove(String input) {
        assertThrows(IllegalArgumentException.class, () -> out.remove(input));
    }
